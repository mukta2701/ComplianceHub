...
type Target = { targetType: "assessment_question"; targetId: string; sessionId: string } | { targetType: "soa_item"; targetId: string } | { targetType: "audit"; targetId: string } | { targetType: "readiness_report" } | { targetType: "task"; targetId: string } | { targetType: "evidence"; targetId: string };
...
export function AiSuggestionPanel({ target }: { target: Target }) {
  ...
  return <aside aria-label="AI Explain and Act" style={{ marginTop: "12px", padding: "12px", border: "1px solid #cfe0fb", borderRadius: "6px", background: "#fbfdff" }}>
    ...
    {draft && <div style={{ marginTop: "9px", fontSize: "12px", color: "#596273", lineHeight: 1.5 }}>
      ...
      {draft.status === "draft" && <span style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}><button type="button" className="button secondary" onClick={() => void review("accepted")}>Mark draft reviewed</button><button type="button" className="button secondary" onClick={() => void review("dismissed")}>Dismiss draft</button></span>}
