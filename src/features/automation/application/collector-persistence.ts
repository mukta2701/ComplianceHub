...
export async function persistCollectedAutomation({
  ...
  const { data: existingSignal, error: existingSignalError } = await supabase.from("automation_signals")
    .select("id").eq("source_object_id", sourceObject.id).maybeSingle();
  ...
  if (signalError?.code === "23505") return false;
  ...
  const { error: linkError } = await supabase.from("automation_proposal_sources").insert({ proposal_id: proposal.id, source_object_id: sourceObject.id, organisation_id: organisationId });
