...
export function evaluateGitHubRepository(
  ...
  return [
    visibility,
    archive,
    evaluateDataRule(facts, context, facts.branchProtection, {
    ...
    evaluateDataRule(facts, context, facts.dependabot, {
    ...
    evaluateDataRule(facts, context, facts.codeScanning, {
    ...
    evaluateDataRule(facts, context, facts.secretScanning, {
    ...
    evaluateDataRule(facts, context, facts.securityWorkflows, {
      checkId: "github.workflow.security",
      title: "An approved security workflow is active and successful",
      evaluate: (value) =>
        value.some((workflow) => workflow.active && workflow.latestConclusion === "success")
          ? pass("At least one active security workflow completed successfully.")
          : fail("high", "No active security workflow has completed successfully.", "Enable an approved security workflow and resolve its failures."),
    }),
    evaluateDataRule(facts, context, facts.administration, {
