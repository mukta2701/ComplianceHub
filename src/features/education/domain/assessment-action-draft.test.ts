...
describe("getAssessmentActionDraft", () => {
  it("creates review-only task and risk drafts for a partial answer", () => {
    const draft = getAssessmentActionDraft(question, "partially");
  ...
  it("does not suggest remediation for a yes or scope-based exclusion", () => {
    expect(getAssessmentActionDraft(question, "yes")).toBeNull();
    expect(getAssessmentActionDraft(question, "not_applicable")).toBeNull();
  });
});
