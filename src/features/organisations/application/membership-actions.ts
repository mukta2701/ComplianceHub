...
import { z } from "zod";
import { canManageMembership, membershipRoles, type MembershipRole } from "@/features/organisations/domain/access";
...
export async function changeMemberRoleAction(formData: FormData) {
  ...
  workspaceAccess(membership.role).section("settings").requireManage("change-member-role");
...
export async function updateMemberJobTitleAction(formData: FormData) {
  const { supabase, membership, organisation } = await requireAppContext();
  workspaceAccess(membership.role).section("settings").requireManage("manage-team-member");
  const userId = String(formData.get("userId"));
