...
describe("Azure staging deployment contract", () => {
  ...
  it("gates mutation on the hosted schema and registered canonical GitHub origin", () => {
    ...
    expect(workflow).toContain("20260825124025");
    ...
    expect(deployment).toMatch(/twenty-four pending additive\s+migrations[\s\S]*20260817010000[\s\S]*20260817020000[\s\S]*20260817030000[\s\S]*20260817192458[\s\S]*20260818030000[\s\S]*20260818040000[\s\S]*20260818050000[\s\S]*20260818060000[\s\S]*20260818070000[\s\S]*20260818100000[\s\S]*20260818110000[\s\S]*20260818120000[\s\S]*20260818130000[\s\S]*20260824184627[\s\S]*20260824184628[\s\S]*20260824212223[\s\S]*20260825014236[\s\S]*20260825040825[\s\S]*20260825053718[\s\S]*20260825073650[\s\S]*20260825082411[\s\S]*20260825094343[\s\S]*20260825124025/);
    expect(deployment).toMatch(/REGISTERED_GITHUB_APP_SITE_URL[\s\S]*NEXT_PUBLIC_SITE_URL/);
    expect(releaseChecklist).toMatch(/hosted Supabase[\s\S]*migrations 1–19 were verified before the manual bridge[\s\S]*migrations 20–24 were verified before final/i);
  ...
  it("documents strict-by-default application mode and the explicit two-stage bridge procedure", () => {
    ...
    expect(deployment).toMatch(/20260825040825[\s\S]*manual[\s\S]{0,200}bridge[\s\S]*policy-capable[\s\S]*20260825053718[\s\S]*20260825073650[\s\S]*20260825082411[\s\S]*20260825094343[\s\S]*20260825124025[\s\S]*manual[\s\S]{0,200}final[\s\S]*strict/i);
  ...
  it("verifies 3100 ingress and all three new-revision probes before health acceptance", () => {
    const capture = workflow.slice(
      workflow.indexOf("- name: Capture rollback revision"),
      workflow.indexOf("- name: Stage rollback-safe secret slot"),
    );
    const rolloutRevision = workflow.slice(
      workflow.indexOf("- name: Create rollout revision"),
      workflow.indexOf("- name: Verify deployed health"),
    ...
    expect(rolloutRevision.indexOf('test "$deployed_target_port" = "3100"'))
      .toBeLessThan(rolloutRevision.indexOf('test "$health_state" = "Healthy"'));
    expect(rolloutRevision.indexOf("length == 3 and all(.[]; . == 3100)"))
      .toBeLessThan(rolloutRevision.indexOf('test "$health_state" = "Healthy"'));
  });
  ...
  it("restores the captured ingress port before copying the rollback revision", () => {
    ...
    const revisionCopy = rollback.indexOf("az containerapp revision copy");
    expect(rollback).toContain('previous_target_port="${{ steps.rollout.outputs.previous-target-port }}"');
    expect(rollback).toContain('--target-port "$previous_target_port"');
    expect(rollback).toContain('test "$restored_target_port" = "$previous_target_port"');
    expect(ingressRestore).toBeGreaterThan(0);
    expect(revisionCopy).toBeGreaterThan(ingressRestore);
    expect(rollback).not.toContain("--target-port 3000");
  });
