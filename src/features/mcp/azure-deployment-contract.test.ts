...
describe("Azure staging deployment contract", () => {
  ...
  it("gates mutation on the hosted schema and registered canonical GitHub origin", () => {
    ...
    expect(workflow).toContain("20260825073650");
    ...
    expect(deployment).toMatch(/twenty pending additive\s+migrations[\s\S]*20260817010000[\s\S]*20260817020000[\s\S]*20260817030000[\s\S]*20260817192458[\s\S]*20260818030000[\s\S]*20260818040000[\s\S]*20260818050000[\s\S]*20260818060000[\s\S]*20260818070000[\s\S]*20260818100000[\s\S]*20260818110000[\s\S]*20260818120000[\s\S]*20260818130000[\s\S]*20260818140000[\s\S]*20260824184628[\s\S]*20260824212223[\s\S]*20260825014236[\s\S]*20260825040825[\s\S]*20260825053718[\s\S]*20260825073650/);
    expect(deployment).toMatch(/REGISTERED_GITHUB_APP_SITE_URL[\s\S]*NEXT_PUBLIC_SITE_URL/);
    expect(releaseChecklist).toMatch(/hosted Supabase[\s\S]*migrations 1–18 were verified before the manual bridge[\s\S]*migrations 19–20 were verified before final/i);
  ...
  it("documents strict-by-default application mode and the explicit two-stage bridge procedure", () => {
    ...
    expect(deployment).toMatch(/20260825040825[\s\S]*manual[\s\S]{0,200}bridge[\s\S]*policy-capable[\s\S]*20260825053718[\s\S]*20260825073650[\s\S]*manual[\s\S]{0,200}final[\s\S]*strict/i);
