...
export function parseMcpOAuthEnvironment(environment: Record<string, string | undefined>): McpOAuthConfig {
  try {
    ...
    const supabaseUrl = canonicalUrl(environment.NEXT_PUBLIC_SUPABASE_URL ?? (production ? "" : "http://127.0.0.1:54321"), { production });
    const expectedIssuer = `${supabaseUrl}/auth/v1`;
    const authorizationServer = canonicalUrl(environment.SUPABASE_OAUTH_ISSUER ?? (production ? "" : expectedIssuer), { production, path: "/auth/v1" });
