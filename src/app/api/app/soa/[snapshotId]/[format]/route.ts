...
export async function GET(_: Request, { params }: { params: Promise<{ snapshotId: string; format: string }> }) {
  ...
  const catalogue = one(data.catalogue_versions);
  const items = data.items as Array<{ controlCode: string; status: string; justification: string; evidence: string }>;
  const view: SoaExportView = {
    title: "Statement of Applicability",
    organisationName: data.organisation_name,
    catalogueVersion: catalogue?.version ?? "unknown",
    version: data.version,
    assessmentId: data.assessment_session_id,
    finalisedAt: data.finalised_at,
    finalisedBy: data.finalised_by,
    items: items.map((item) => ({
      reference: item.controlCode,
  ...
  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      ...
      "content-disposition": "attachment; filename=\"statement-of-applicability-v" + data.version + "." + format + "\"",
