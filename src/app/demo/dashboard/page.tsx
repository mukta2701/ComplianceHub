...
const cats = [["Organisational",50,"blue"],["People",50,"violet"],["Physical",0,"green"],["Technological",67,"amber"]] as const;
const gaps = [
...
export default function Dashboard() {
  return <><PageIntro eyebrow="FRIDAY, 4 SEPTEMBER" title="Good evening, Priya" body="Here’s where Northstar Labs stands on ISO 27001 readiness." action={<Link className="button primary" href="/demo/assessment">Continue assessment <Icon name="arrow"/></Link>}/>
    <div className="stats-grid"><Stat label="ASSESSMENT" value="50%" detail="5 of 10 answered"/><Stat label="PRIORITY GAPS" value="5" detail="2 need urgent attention" tone="amber"/><Stat label="OPEN RISKS" value="4" detail="3 high or critical" tone="red"/><Stat label="SOA REVIEW" value="6" detail="controls need review" tone="green"/></div>
    <div className="dashboard-grid"><Card className="readiness-card"><div className="card-head"><div><h3>Overall readiness</h3><p>Based on your latest answers</p></div><Pill>Draft assessment</Pill></div><div className="readiness-body"><Ring value={50}/><div className="category-bars">{cats.map(([name,value,tone])=><div key={name}><label><span>{name}</span><b>{value}%</b></label><Progress value={value} tone={tone}/></div>)}</div></div><div className="card-foot"><span><Icon name="clipboard"/>Assessment 50% complete</span><Link href="/demo/assessment">Review answers <Icon name="arrow"/></Link></div></Card>
