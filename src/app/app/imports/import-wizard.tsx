  ...
  return <>
    <Card style={{ padding: "22px", marginBottom: "16px" }}>
      ...
      <form action={(fd) => start(async () => analyse(fd))} style={{ display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        {registers && registers.length > 0 && <label style={{ fontSize: "13px", fontWeight: 700 }}>Register<select value={registerId} onChange={(e) => { setRegisterId(e.target.value); setPreview(null); setResult(null); }} disabled={pending} className="field" style={{ display: "block" }} aria-label="Target SoA register">{registers.map((r) => <option key={r.id} value={r.id}>{r.title}</option>)}</select></label>}
        <input className="import-workbook-input" name="file" type="file" accept=".xlsx,.csv" required disabled={pending} onChange={() => { setHeaders(null); setRows([]); setMapping({}); setPreview(null); setResult(null); setError(null); }} aria-label="Workbook file (XLSX or CSV)" />
    ...
    {headers && <Card style={{ padding: "22px", marginBottom: "16px" }}>
      ...
      <div className="import-column-mapping" role="region" aria-label="Column mapping">
        <div className="import-column-header" aria-hidden="true"><span>File column</span><span>Maps to</span></div>
        {headers.map((h) => <label className="import-column-row" key={h}>
          <span>{h}</span>
          <select aria-label={`Map column ${h}`} className="field" value={mapping[h] ?? ""} disabled={pending} onChange={(e) => { setMapping({ ...mapping, [h]: e.target.value }); setPreview(null); setResult(null); }}>
