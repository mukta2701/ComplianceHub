...
export default async function ActivityPage() {
  ...
  return <>
    ...
    <Card>{data?.length ? data.map((e) => { const profile = one(e.profiles); return <div style={{ padding: "14px 18px", borderTop: "1px solid #edf0f4", fontSize: "13px" }} key={e.id}><b style={{ textTransform: "capitalize" }}>{e.action}</b> <span style={{ textTransform: "capitalize" }}>{e.entity_type.replaceAll("_", " ")}</span><span style={{ float: "right", color: "#596273" }}>{profile?.display_name ?? "System"} · {new Date(e.occurred_at).toLocaleString("en-GB")}</span></div>; }) : <p style={{ padding: "20px", color: "#596273" }}>No recorded activity yet.</p>}</Card>
  </>;
}
