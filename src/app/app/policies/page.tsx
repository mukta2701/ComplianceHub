  ...
  return <div className={styles.workspace}>
    ...
    {policyResult.error ? <Card className={styles.panel}><h2>Policy library unavailable</h2><p role="alert">We could not load your policies. Refresh to try again; this does not mean the library is empty.</p></Card> : <>
      <div className={styles.summaryGrid}>
        ...
        <div className={styles.summary}><span>{access.canManage ? "In review" : "My acceptances"}</span><strong>{access.canManage ? rows.filter((p) => p.status === "in_review").length : acceptanceUnavailable ? "Unavailable" : personallyAccepted}</strong></div>
      </div>
      <Card className={styles.register}>
        <header className={styles.registerHeader}><div><h2>All policies</h2><p>{access.canManage ? "Ownership, review dates and acceptance of the current approved version." : "Your acceptance is specific to the version you read."}</p></div></header>
