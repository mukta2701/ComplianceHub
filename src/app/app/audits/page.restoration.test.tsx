...
describe("AuditsPage restored preflight", () => {
  ...
  it("renders deterministic blockers and keeps planning unavailable to Members", async () => {
    ...
    expect(screen.getByText(/Non-conformities/i).closest("section")).toHaveTextContent("1minor or major, still open");
  ...
  it("marks finding metrics unavailable while preserving the loaded audit register", async () => {
    ...
    expect(screen.getAllByText("Access review")).toHaveLength(2);
    expect(screen.getByText(/Open audits/i).closest("section")).toHaveTextContent("1");
    expect(screen.getByText(/Open findings/i).closest("section")).toHaveTextContent(/Unavailable/);
    expect(screen.getByText(/Non-conformities/i).closest("section")).toHaveTextContent(/Unavailable/);
  ...
  it("shows zero finding metrics for a successful empty findings result", async () => {
    ...
    expect(screen.getByText(/Open findings/i).closest("section")).toHaveTextContent("0awaiting closure");
    expect(screen.getByText(/Non-conformities/i).closest("section")).toHaveTextContent("0minor or major, still open");
