...
export default async function SoaReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (requirementIds.length) {
    ...
    if (sharedControlIds.length) {
      const { data: tasks } = await supabase.from("tasks").select("id,control_id,status").in("status", ["open", "in_progress"]).in("control_id", sharedControlIds);
