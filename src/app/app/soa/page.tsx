...
export default async function SoaPage() {
  ...
  return <>
    ...
    {membership.role === "member" ? <Card style={{ padding: "18px" }}><b>Review access only</b><p style={{ margin: "6px 0 0", color: "#596273", fontSize: "13px" }}>You can open existing drafts and download finalised statements. A workspace operator must generate or finalise a statement.</p></Card> : assessments?.length ? (
      <Card style={{ padding: "16px" }}><form action={createSoaAction} style={{ display: "flex", gap: "12px" }}><label htmlFor="soa-assessment" style={{ display: "flex", alignItems: "center", color: "#596273", fontSize: "12px", fontWeight: 700 }}>Start from</label><select id="soa-assessment" name="assessmentId" required className="field" style={{ flex: 1 }}><option value="">Select an assessment</option>{assessments.map((a) => <option key={a.id} value={a.id}>{a.title}</option>)}</select><button className="button primary">Generate draft</button></form></Card>
    ) : (
