...
export default async function NewEvidencePage({ searchParams }: { searchParams: Promise<{ replaces?: string; message?: string }> }) {
  ...
  const [{ data: members, error: membersError }, replacementResult] = await Promise.all([
    ...
    replacementId ? supabase.from("evidence").select("id,title,status,collected_on,valid_until").eq("id", replacementId).eq("organisation_id", organisation.id).in("status", ["current", "expiring", "expired"]).maybeSingle() : Promise.resolve({ data: null, error: null }),
  ]);
  ...
  const replacementUnavailable = Boolean(replacementId && !replacement);
  return <div className={styles.page}>
    ...
    {(message || replacementUnavailable) && <p role="alert" className={styles.truthNote}>{message ?? "That evidence record is unavailable or already in history. A new record can still be added without replacing it."}</p>}
    <div className={styles.formLayout}>
    <form action={createEvidenceAction} className={`${styles.formCard} card app-form`}>
      {replacement?.id && <input type="hidden" name="replacesEvidenceId" value={replacement.id} />}
      ...
      <section className={styles.formSection}><div className={styles.sectionHeading}><span className={styles.sectionNumber}>2</span><div><h2>Add the source</h2><p>Choose how this proof is recorded. The form only asks for the source information that will be saved.</p></div></div>
      <EvidenceSourceFields helpClassName={styles.fieldHelp} /></section>
