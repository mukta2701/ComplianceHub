...
import { automationConnectionId } from "@/features/automation/application/collector-persistence";
...
function connectionDetail(connection: { status: string; last_collected_at?: string | null; last_error_at?: string | null }) {
  const collected = connection.last_collected_at ? `Last collected ${formatCollectedAt(connection.last_collected_at)}` : null;
  const failedAfterCollection = connection.last_error_at && (!connection.last_collected_at || new Date(connection.last_error_at) >= new Date(connection.last_collected_at));
  if (connection.status === "revoked") return "Connection revoked; reconnect to collect";
  if (connection.status === "setup") return "Pending setup; collection is not ready";
  if (connection.status === "paused") return "Collection paused";
  if (connection.status === "error" || failedAfterCollection) return `Collection error; needs attention${collected ? ` · ${collected}` : ""}`;
  return collected ?? "Connected; no collection recorded yet";
}
...
export default async function AutomationPage({ searchParams }: { searchParams: Promise<{ message?: string }> }) {
  ...
  const queryError = connectionsError || proposalsError || aiSettingsError || aiDraftsError || sourcesError;
  const runnableSourceIds = new Set((sources ?? []).map((source) => source && source.config && typeof source.config === "object" && !Array.isArray(source.config) ? automationConnectionId(source.config as Record<string, unknown>) : null).filter((id): id is string => Boolean(id)));
  const canGenerateBaseline = membership.role === "owner" && !queryError && runnableSourceIds.size > 0;
  ...
  return <>
    <PageIntro eyebrow="AUTOMATION" title="Review the work your systems prepared" body="Configured systems collect bounded evidence, map it to GRC work, and prepare drafts. Nothing becomes a compliance decision until the assigned owner reviews it." action={<span style={{ display: "flex", gap: "8px" }}>{canGenerateBaseline && <form action={generateAutomationBaselineAction}><button className="button primary">Generate baseline</button></form>}<Link className="button secondary" href="/app/setup">Edit setup</Link></span>} />
    {message && <Card role="status" style={{ padding: "12px", marginBottom: "16px", background: "#f0f7ff", borderColor: "#cfe0fb" }}>{message}</Card>}
    {queryError && <Card role="alert" style={{ padding: "12px", marginBottom: "16px", background: "#fff8f0", borderColor: "#f0d4ad" }}>Could not load automation status. Refresh to try again; the values below may be incomplete.</Card>}
    <div className="stats-grid"><Stat label="CONNECTED SYSTEMS" value={connectionsError ? "—" : (connections ?? []).filter((connection) => connection.status === "connected").length} detail={connectionsError ? "status unavailable" : `${connections?.length ?? 0} configured`} tone="blue" /><Stat label="YOUR REVIEWS" value={proposalsError ? "—" : mine.length} detail={proposalsError ? "status unavailable" : "drafts waiting for you"} tone={proposalsError ? "blue" : mine.length ? "amber" : "green"} /><Stat label="ALL DRAFTS" value={proposalsError ? "—" : drafts.length} detail={proposalsError ? "status unavailable" : "human review required"} tone={proposalsError ? "blue" : drafts.length ? "amber" : "green"} /></div>
    <Card style={{ padding: "20px", marginTop: "16px" }}>
      <h2 style={{ fontSize: "15px", margin: "0 0 10px" }}>Connection health</h2>
      {connectionsError ? <p style={{ margin: 0, color: "#596273", fontSize: "13px" }}>Connection health is unavailable until the status can be loaded.</p> : (connections ?? []).length ? <div style={{ display: "grid", gap: "10px" }}><p style={{ margin: 0, color: "#596273", fontSize: "12px" }}>{process.env.EVIDENCE_LIVE === "1" ? "Live collection is enabled. Connection status alone does not prove that provider access works." : "Fictional sandbox collection — does not contact providers."}</p>{connections?.map((connection) => <div key={connection.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", fontSize: "13px" }}><span><b>{connection.label || connection.provider}</b><small style={{ display: "block", color: "#596273", marginTop: "3px" }}>{connectionDetail(connection)}</small></span><span style={{ display: "flex", gap: "10px", alignItems: "center" }}><Pill tone={connection.status === "connected" ? "green" : connection.status === "error" ? "red" : connection.status === "revoked" ? "red" : "amber"}>{connection.status}</Pill>{membership.role === "owner" && connection.status !== "revoked" && <form action={revokeAutomationConnectionAction}><input type="hidden" name="id" value={connection.id} /><button type="submit" style={{ color: "var(--red)", fontWeight: 700, border: 0, background: "none" }}>Disconnect</button></form>}</span></div>)}</div> : <p style={{ margin: 0, color: "#596273", fontSize: "13px" }}>No automation systems are configured yet.</p>}
      {!queryError && !runnableSourceIds.size && <p style={{ margin: "10px 0 0", color: "#596273", fontSize: "13px" }}>Configure an automation source before running a baseline.</p>}
    </Card>
    <section style={{ marginTop: "22px" }} aria-labelledby="automation-inbox"><h2 id="automation-inbox" style={{ fontSize: "16px", margin: "0 0 10px" }}>Automation inbox</h2>
      ...
      {proposalsError ? <Card role="status" style={{ padding: "14px" }}>Automation drafts are unavailable until this workspace can be read.</Card> : !drafts.length && <EmptyState icon="clipboard" title="No automation drafts yet" body="Complete setup, then run a baseline collection. Evidence and remediation drafts will appear here for the people responsible for the work." primary={{ href: "/app/setup", label: "Set up automation" }} />}
