...
export default async function NewRiskPage({ searchParams }: { searchParams: Promise<{ title?: string; description?: string; treatmentPlan?: string; sourceAssessmentSessionId?: string }> }) {
  ...
  return <>
    ...
    <form action={createRiskAction} className="card app-form">
      ...
      <label>Description<textarea name="description" required placeholder="Risk description" defaultValue={suggested.description ?? ""} /></label>
      <div className="form-grid">
      ...
      <label>Treatment plan<textarea name="treatmentPlan" placeholder="Treatment plan" defaultValue={suggested.treatmentPlan ?? ""} /></label>
