...
export default async function AssessmentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><h1 className="text-3xl font-bold">{session.title}</h1><p className="mt-2 text-slate-600">Answers save automatically. A conflict is shown rather than overwriting newer work.</p><div className="mt-8"><AssessmentResponseList sessionId={id} questions={questions??[]} initialRevision={session.revision} responses={responses??[]}/></div></>;
}
