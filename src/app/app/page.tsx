...
export default async function AppHome() {
  ...
  return <main className="mx-auto max-w-6xl px-6 py-12">
    ...
    <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[["Assessments",assessments,"/app/assessment"],["Open risks",risks,"/app/risks"],["Finalised SoAs",snapshots,"/app/soa"],["Open tasks",openTasks,"/app/tasks"]].map(([label,value,href]) => <Link key={label} href={String(href)} className="rounded-xl border bg-white p-5 shadow-sm"><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-3xl font-bold">{value ?? 0}</p></Link>)}</div>
