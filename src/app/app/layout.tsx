...
export default async function ProtectedLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-950">
    <header className="border-b border-slate-200 bg-white"><div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
      ...
      <nav aria-label="Workspace" className="flex max-w-[55vw] gap-3 overflow-x-auto text-xs md:max-w-none md:gap-5 md:text-sm"><Link href="/app/assessment">Assessment</Link><Link href="/app/soa">SoA</Link><Link href="/app/risks">Risks</Link><Link href="/app/tasks">Tasks</Link><Link href="/app/activity">Activity</Link><Link href="/app/settings">Settings</Link></nav></div>
