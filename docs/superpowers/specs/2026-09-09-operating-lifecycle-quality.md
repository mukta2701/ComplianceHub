...
Policy save recovery also covers transport failure: retain entered fields and say the outcome cannot be confirmed. Keep the draft's expected version fixed through unrelated canonical refreshes; advance it only from the version returned by that draft's own successful database update. This closes a demonstrated stale-draft overwrite path without changing the policy approval model.

## Local resource recovery — owner steering on 9 September

The owner requested repeated Mac crashes be addressed before broader work resumes. Preserve source, local records and personal files. Clear only identified regenerable build caches, retain database volumes, and prefer an independently running production preview for browser review. A local command guard refuses startup below 8 GiB available disk, stops its own process group below 6 GiB free or above 4 GiB resident memory, and supplies a 2 GiB V8 old-space setting. Graceful interruption escalates only for that group; unrelated applications remain untouched. A failed resource probe stops work. Use sequential heavy workloads and safe CLI tests with artificial thresholds, never deliberate system exhaustion. Document sampled protection and native/compressed-memory limitations; do not claim all shutdown causes resolved. The guard does not choose credentials or database targets. Background app running does not mean automatic recovery after an OS shutdown.
