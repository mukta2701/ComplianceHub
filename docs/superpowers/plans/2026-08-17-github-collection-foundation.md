...
export type CollectionDependencies = {
  ...
  listPersistedObservations(reservation: RunReservation, target: CollectionTarget): Promise<Array<{ observationKey: string; result: "pass" | "fail" | "unknown" | "not_applicable" }>>;
...
`RunReservation` carries the run ID, unguessable lease token, acquisition state, status, lease expiry, attempt, and organisation/installation/repository/provider-repository ancestry returned by a tenant-scoped service RPC. Add lease columns and service-only claim/refresh/save/finalize RPCs: initial reservation inserts `running`; an active running duplicate is exposed as a distinct deferred/in-progress summary count; one worker may compare-and-set reclaim an expired lease; a completed duplicate remains immutable. Use a lease comfortably above the 15-second token plus 90-second collector budget (while below the 270-second route deadline), or renew it. Every inventory refresh, observation insert, and finalization requires the current lease token, attempt, and complete ancestry; finalization succeeds only from `running` with exactly one row updated. If a reclaimed run already has the full immutable expected observation set, derive its result counts and finalize it without GitHub recollection; if it has none, recollect; any partial/unexpected set fails safely for manual review. The persisted-count result is the complete count for the run, not only newly inserted rows. A scheduled request therefore reserves one run per target repository instead of colliding globally. Collection history is append-only; repository inventory fields refresh only after verified metadata; a failure never extends observation freshness.

`CollectionTarget` must distinguish local UUIDs (`installationId`, `repositoryId`) from GitHub numeric IDs (`providerInstallationId`, `providerRepositoryId`) and also carry `organisationId`, `owner`, and `name`. Target loading paginates all selected rows, accepts only active `permissions_ok` installations and selected available repositories, and enforces at most 100 per installation rather than globally; scoped IDs and complete ancestry must match; one local installation maps to exactly one provider installation; duplicate local or provider repository identities fail before a GitHub call. Export the exact expected check-ID set from the rule pack and validate a complete, distinct observation set before one lease-checked bulk insert: reject missing, duplicate, unexpected, wrong-run, or wrong-provider-repository observations. After stable numeric repository-ID verification, allow GitHub rename/case refresh: derive expected subject/source/observation keys from the verified fact identity, refresh inventory under the current lease, then save. Group targets explicitly by installation rather than trusting query order. A typed safe `GitHubCollectionError` with `diagnosticCode: "rate_limited"` from either installation-token exchange or repository APIs finalises the current run as rate-limited, skips only the remaining targets for that installation, and continues other installations. A compliance `fail` remains a successfully collected run; any `unknown` makes the run `partial`, including recovered persisted sets; `not_applicable` alone remains `succeeded`. Every lease-checked finalization must return and verify `true`.
...
Validate delivery IDs as 1–100 safe printable ASCII characters without whitespace/control (do not assume UUID version bits), validate bounded event headers, and fail closed when the webhook secret is absent. Read the request stream with a hard 1 MiB bound before JSON parsing; reject a larger declared or streamed body with `413` and cancel the reader; calculate HMAC-SHA256 over the exact retained bytes; compare a strict lowercase `sha256=` plus 64-hex signature using equal-length digest buffers and `timingSafeEqual`; decode UTF-8 fatally; hash but do not store the body. Allow `installation`, `installation_repositories`, `repository`, `branch_protection_rule`, `repository_ruleset`, `workflow_run`, `dependabot_alert`, `code_scanning_alert`, and `secret_scanning_alert`. Treat sender/repository names as untrusted text and accept routing IDs only when they are actual JSON numbers that are positive safe integers. `repository_ruleset` is dual-scope: installation is required; a present repository ID is validated and scoped, while a missing repository remains installation-scoped. Intake performs one INSERT and no pre-read/upsert: supported events omit all tenant/local IDs and use queued defaults; unsupported events store only delivery/event/hash plus terminal ignored state. Only database `23505` is duplicate `202`; other storage failures remain generic retryable 5xx.

Add a bounded worker to the existing `github-collect` cron path before scheduled reconciliation. It calls `claim_github_webhook_deliveries_server` once per invocation with a small fixed cap—never refill-looping failed rows—parses claimed rows independently so one malformed legacy row cannot poison valid neighbours, then processes each delivery independently. A missing resolved installation is ignored; a stored provider repository ID with no selected/available local repository is ignored and never broadened; only a null provider repository ID may remain installation-scoped. Invoke Task 6 with local UUIDs and the fixed delivery request key. Give webhook drain a dedicated sub-budget and check abort before and after each delivery; aborted/deferred work must remain retryable and never become processed. Run scheduled reconciliation afterward with a fresh bounded signal and reserved time under the route's 270-second total, even if draining throws. A resolved Task 6 summary with zero deferred/in-progress repositories, including partial/repository-failure counts, means the delivery is processed because terminal run keys are immutable; any deferred active duplicate keeps the delivery retryable until the owning run becomes terminal. Daily scheduled collection is the recovery path for terminal repository failures. Thrown safe typed failures may become allowlisted diagnostics; unknowns become `internal_error`; every compare-and-set finalizer must return `true`, while `false` means the worker lost ownership and stops touching the row. Never trust payload timestamps as replay proof, and never store/log a raw body, header, provider/database error, token, ID, or stack.
