...
- [ ] Hosted Supabase personal staging project ref `ytenjiyjdcrjkgwmciqw` has a verified backup; `supabase migration list` and `supabase db push --dry-run` showed exactly the nineteen reviewed migrations through `20260825053718` in order; migrations 1–18 were verified before the manual bridge, additive migration 19 was verified before final, and the phase-specific `HOSTED_SUPABASE_MIGRATION_VERSION` always matched `NEXT_PUBLIC_SUPABASE_URL`.
...
- [ ] The staged Slack rollout completed in order: manual `bridge` on migration `20260825040825`, health capability `v1`/`bridge` with the exact release SHA, additive migration `20260825053718`, then manual `final` in `strict` mode. Automatic runs stayed final/strict, final had exact same-slot Slack/core references, and rollback was pinned to the captured revision/image/references.
