...
select lives_ok($$ select public.issue_invitation(current_setting('app.org_a')::uuid,'new-member@example.test','member','Engineer',repeat('1',64),now()+interval '1 day') $$,'admin can invite a member');
select throws_ok($$ select public.issue_invitation(current_setting('app.org_a')::uuid,'new-admin@example.test','admin',null,repeat('2',64),now()+interval '1 day') $$,'42501','your role cannot invite that role','admin cannot invite an admin');
select throws_ok($$ select public.issue_invitation(current_setting('app.org_a')::uuid,'new-owner@example.test','owner',null,repeat('3',64),now()+interval '1 day') $$,'42501','owner invitations are not permitted','admin cannot issue an owner invitation');
select set_config('request.jwt.claims','{"sub":"61000000-0000-4000-8000-000000000001","email":"roles-owner-a@example.test","role":"authenticated"}',true);
select lives_ok($$ select public.issue_invitation(current_setting('app.org_a')::uuid,'owner-created-admin@example.test','admin',null,repeat('5',64),now()+interval '1 day') $$,'owner can invite an admin');
set local role postgres;
