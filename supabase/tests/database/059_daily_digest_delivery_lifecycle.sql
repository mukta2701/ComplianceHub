...
select has_function('public','reserve_daily_digest_delivery_server',array['uuid','uuid','date','text','jsonb'],'server reservation requires an explicit verified actor');
...
select ok(not has_function_privilege('authenticated','public.reserve_daily_digest_delivery_server(uuid,uuid,date,text,jsonb)','EXECUTE'),'authenticated cannot supply a forged reservation actor');
...
select ok(not has_function_privilege('anon','public.reserve_daily_digest_delivery_server(uuid,uuid,date,text,jsonb)','EXECUTE'),'anon cannot reserve');
...
select ok(not has_function_privilege('public','public.reserve_daily_digest_delivery_server(uuid,uuid,date,text,jsonb)','EXECUTE'),'PUBLIC cannot reserve');
...
select ok(has_function_privilege('service_role','public.reserve_daily_digest_delivery_server(uuid,uuid,date,text,jsonb)','EXECUTE'),'only the backend service capability may reserve');
...
select throws_ok(
  $$ select public.reserve_daily_digest_delivery_server(current_setting('app.lifecycle_org')::uuid,'86000000-0000-4000-8000-000000000001','2026-08-07',repeat('a',64),'{"text":"forged","blocks":[]}') $$,
...
select throws_ok(
  $$ select public.reserve_daily_digest_delivery_server(current_setting('app.lifecycle_org')::uuid,'86000000-0000-4000-8000-000000000002','2026-08-07',repeat('a',64),'{"text":"Daily","blocks":[]}') $$,
  '42501','daily digest action requires a workspace Owner','the backend rejects an Admin actor'
);
select throws_ok(
  $$ select public.reserve_daily_digest_delivery_server(current_setting('app.lifecycle_org')::uuid,'86000000-0000-4000-8000-000000000003','2026-08-07',repeat('a',64),'{"text":"Daily","blocks":[]}') $$,
  '42501','daily digest action requires a workspace Owner','the backend rejects a Member actor'
);
select throws_ok(
  $$ select public.reserve_daily_digest_delivery_server(current_setting('app.lifecycle_org')::uuid,'86000000-0000-4000-8000-000000000004','2026-08-07',repeat('a',64),'{"text":"Daily","blocks":[]}') $$,
...
select is((public.reserve_daily_digest_delivery_server(
  current_setting('app.lifecycle_other_org')::uuid,'86000000-0000-4000-8000-000000000004','2026-08-07',repeat('a',64),'{"text":"Daily","blocks":[]}'
...
select set_config('app.first_reservation',public.reserve_daily_digest_delivery_server(
  current_setting('app.lifecycle_org')::uuid,'86000000-0000-4000-8000-000000000001','2026-08-07',repeat('a',64),
...
set local role service_role;
select is((public.reserve_daily_digest_delivery_server(
  current_setting('app.lifecycle_org')::uuid,'86000000-0000-4000-8000-000000000001','2026-08-07',repeat('a',64),'{"text":"Concurrent","blocks":[]}'
...
select is((public.reserve_daily_digest_delivery_server(
  current_setting('app.lifecycle_org')::uuid,'86000000-0000-4000-8000-000000000001','2026-08-07',repeat('a',64),'{"text":"New","blocks":[]}'
...
select set_config('app.disabled_reservation',public.reserve_daily_digest_delivery_server(
  current_setting('app.lifecycle_org')::uuid,'86000000-0000-4000-8000-000000000001','2026-08-08',repeat('b',64),'{"text":"Channel changed","blocks":[]}'
)::text,true);
select ok(public.finalize_daily_digest_delivery_server(
