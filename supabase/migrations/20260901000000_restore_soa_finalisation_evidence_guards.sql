-- Some upgraded installations retained a pre-evidence-guard finalise_soa
-- definition after the member-to-operator hardening rewrite. Reassert the
-- complete finalisation contract explicitly for both upgrade and fresh paths.
...
begin
  ...
  if not found or not public.is_organisation_operator(register_row.organisation_id) then
  ...
  if exists (
    select 1 from public.soa_items as item
    where item.soa_register_id = target_register_id
      and item.organisation_id = register_row.organisation_id
      and item.applicable
  ...
  select
    register.organisation_id, register.id, register.assessment_session_id,
    session.catalogue_version_id, register.control_catalogue_version_id,
    register.version, organisation.name, register.title,
