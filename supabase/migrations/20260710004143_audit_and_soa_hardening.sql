-- A finalised SoA is an immutable audit record. Applicable controls must have
-- a named accountable owner, and the snapshot must preserve that assignment.
...
begin
  ...
  if exists(select 1 from public.soa_items where soa_register_id=target_register_id and applicable and owner_id is null) then raise exception 'Every applicable SoA item requires an owner before finalisation'; end if;
  ...
  select r.organisation_id,r.id,r.assessment_session_id,s.catalogue_version_id,r.control_catalogue_version_id,r.version,o.name,r.title,
    jsonb_agg(jsonb_build_object('controlCode',i.control_code,'controlTitle',i.control_title,'applicable',i.applicable,'status',i.status,'ownerId',i.owner_id,'justification',i.justification,'evidence',i.evidence) order by i.position),(select auth.uid())
