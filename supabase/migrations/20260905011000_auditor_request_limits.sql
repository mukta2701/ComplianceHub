-- Apply limits inside the anonymous RPC, before access logging or aggregation.
-- Preserve the existing payload and null-on-unavailable contract for all callers.
...
begin
  -- One fixed global key also bounds invalid/rotating-token traffic. Do not
  -- raise on exhaustion: an exception would roll back the reserved budget.
  ...
  -- Keys are derived from an issued token ID, never caller-selected headers
  -- or arbitrary input, so invalid tokens cannot grow this table unboundedly.
