...
begin
  ...
  -- A browser can replay a server action while hydration settles. Accept the
  -- exact same terminal decision for the same reviewer without duplicating its
  -- evidence/task side effect; every other terminal-state mutation remains
  -- rejected.
  if proposal.status <> 'draft' then
    if proposal.status::text = target_decision and proposal.reviewer_id = actor_id then
      return;
    end if;
    raise exception 'Automation proposal is not available to this reviewer' using errcode = '42501';
  end if;
