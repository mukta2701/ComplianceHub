...
create table public.github_materialisation_jobs (
  ...
  status text not null default 'pending'
    check (status in ('pending', 'awaiting_approval', 'retryable', 'completed')),
...
begin
  ...
  set status = case target_outcome
    ...
    available_at = case target_outcome
      when 'completed' then job.available_at
      when 'awaiting_approval' then 'infinity'::timestamptz
    ...
    completed_at = case target_outcome when 'completed' then pg_catalog.now() else null end,
    updated_at = pg_catalog.now()
