...
export async function main() {
  ...
  try {
    ...
    async function submit(button: string) {
      const response = page.waitForResponse((r) => r.request().method() === 'POST' && new URL(r.url()).origin === site);
      await page.getByRole('button', { name: button, exact: true }).click();
      const r = await response; if (r.status() >= 400) throw new Error(`App action failed: ${button} (${r.status()})`);
    ...
    await page.goto('/sign-in'); await page.getByLabel('Email', { exact: true }).fill(credentials.email); await page.getByLabel('Password', { exact: true }).fill(credentials.password); await submit('Sign in');
    ...
    const membership = await db.from('memberships').select('user_id,role').eq('organisation_id', org.id);
    if (membership.error || membership.data?.length !== 1 || membership.data[0].user_id !== user.id || membership.data[0].role !== 'owner') throw new Error('Showcase membership drift');
    ...
    async function ensure(key: string, table: string, filters: Record<string, string>, expected: Record<string, unknown>, create: () => Promise<void>) {
      ...
      if (!record) { if (verifyOnly) throw new Error(`Missing showcase ${key}`); console.log(`Creating ${key}`); await create(); record = reconcileRecord(await rows(table, filters), undefined, expected); if (!record) throw new Error(`Creation failed: ${key}`); }
    ...
    const qResult = await db.from('catalogue_questions').select('id,code,position').eq('catalogue_version_id', assessment.catalogue_version_id).order('position'); if (qResult.error || !qResult.data?.length) throw new Error('Missing assessment catalogue');
    ...
    const firstItem = soaItems.sort((a, b) => a.position - b.position)[0]; if (!firstItem) throw new Error('Missing SoA items');
    ...
    if (firstItem.justification !== SPEC.note || firstItem.owner_id !== user.id || firstItem.status !== 'in_progress') {
      ...
      await page.getByLabel('Implementation status', { exact: true }).selectOption('in_progress'); await page.getByLabel('Owner assignment').selectOption(user.id); await page.getByLabel('Rationale', { exact: true }).fill(SPEC.note); await submit('Save draft');
    ...
    const risk = await ensure('risk', 'risks', { reference: SPEC.riskRef }, { title: SPEC.riskTitle, owner_id: user.id, source_assessment_session_id: assessment.id }, async () => {
      ...
      await page.getByLabel('Category', { exact: true }).selectOption({ index: 1 }); await page.getByLabel('Owner', { exact: true }).selectOption(user.id); await submit('Save risk');
    });
    ...
    const evidence = await ensure('evidence', 'evidence', { title: SPEC.evidenceTitle }, { owner_id: user.id, kind: 'note', description: SPEC.note }, async () => {
      await page.goto('/app/evidence/new'); await page.getByLabel('Title', { exact: true }).fill(SPEC.evidenceTitle); await page.getByLabel('Kind').selectOption('note'); await page.getByLabel('Description', { exact: true }).fill(SPEC.note); await page.getByLabel('Owner', { exact: true }).selectOption(user.id); await page.getByLabel('Collected on').fill('2026-09-05'); await page.getByLabel('Valid until').fill('2027-09-05'); await submit('Save evidence');
    });
    for (const [kind, target] of [['control', control.id], ['risk', risk.id], ['task', task.id]]) {
      await ensure(`evidence_${kind}`, 'evidence_links', { evidence_id: evidence.id, [`${kind}_id`]: target }, {}, async () => {
        await page.goto('/app/evidence'); const select = page.getByLabel(`Link ${SPEC.evidenceTitle} to a control`); await select.selectOption(`${kind}:${target}`); const form = select.locator('..'); await form.getByRole('button', { name: 'Link', exact: true }).click(); await page.waitForLoadState('networkidle');
      });
    }
    const policy = await ensure('policy', 'policies', { reference: SPEC.policyRef }, { title: SPEC.policyTitle, owner_id: user.id, body: SPEC.note }, async () => {
      await page.goto('/app/policies/new'); await page.getByLabel('Reference', { exact: true }).fill(SPEC.policyRef); await page.getByLabel('Title', { exact: true }).fill(SPEC.policyTitle); await page.getByLabel('Owner', { exact: true }).selectOption(user.id); await page.getByLabel('Review due').fill(SPEC.due); await page.getByLabel('Policy content').fill(SPEC.note); await submit('Create policy');
    });
    ...
    for (const table of ['assessment_sessions', 'assessment_responses', 'soa_registers', 'soa_items', 'risks', 'risk_treatment_plans', 'tasks', 'evidence', 'evidence_links', 'policies', 'audits', 'audit_checklist_items', 'audit_findings', 'leadership_report_snapshots']) { const current = await rows(table); manifest.counts[table] = current.length; manifest.fingerprints[table] = fingerprintRows(current); }
    for (const [name, url] of Object.entries({ 'readiness-report.pdf': '/api/app/reports/readiness/pdf', 'audit-pack.csv': `/api/app/audits/${audit.id}/pack?format=csv`, 'risks.csv': '/api/app/risks/export?format=csv' })) {
