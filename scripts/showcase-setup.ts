...
export async function main() {
  ...
  const browserRequests: { path: string; status?: number; failure?: string }[] = [];
  const sessions: { context: import('@playwright/test').BrowserContext; file: string }[] = [];
  try {
    ...
    const context = await browser.newContext({ baseURL: site, storageState }); const page = await context.newPage(); diagnosticPage = page; sessions.push({ context, file: statePath });
    page.on('console', (message) => { if (message.type() === 'error') browserErrors.push(message.text()); });
    page.on('pageerror', (error) => browserErrors.push(error.message));
    function observeRequests(targetPage: import('@playwright/test').Page) {
      targetPage.on('response', (response) => { browserRequests.push({ path: new URL(response.url()).pathname, status: response.status() }); });
      targetPage.on('requestfailed', (request) => { browserRequests.push({ path: new URL(request.url()).pathname, failure: request.failure()?.errorText }); });
    }
    observeRequests(page);
    page.setDefaultTimeout(30_000);
    async function navigate(targetPage: import('@playwright/test').Page, url: string) { diagnosticPage = targetPage; return targetPage.goto(url, { waitUntil: 'domcontentloaded' }); }
    ...
    if (!user) {
      ...
      await navigate(page, '/sign-up'); await page.getByLabel('Name', { exact: true }).fill(SPEC.owner);
    ...
    await expect(page.getByRole('heading', { name: 'Readiness dashboard', exact: true })).toBeVisible();
    await expect(page.locator('.sidebar .workspace b')).toHaveText(SPEC.organisation);
    ...
    let assessment = await ensure('assessment', 'assessment_sessions', {}, { created_by: user.id }, async () => {
      await navigate(page, '/app/assessment'); await submit('New assessment');
    });
    ...
    const soa = await ensure('soa', 'soa_registers', {}, { assessment_session_id: assessment.id }, async () => {
      await navigate(page, '/app/soa'); await page.locator('select[name="assessmentId"]').selectOption(assessment.id); await submit('Generate draft');
    });
    ...
    if (firstItem.justification !== SPEC.note || firstItem.owner_id !== user.id || firstItem.status !== 'in_progress') {
      ...
      await navigate(page, `/app/soa/${soa.id}`); await page.getByRole('button', { name: `Review ${firstItem.control_code} ${firstItem.control_title}`, exact: true }).click();
    ...
    const risk = await ensure('risk', 'risks', { reference: SPEC.riskRef }, { title: SPEC.riskTitle, owner_id: user.id, source_assessment_session_id: assessment.id }, async () => {
      await navigate(page, `/app/risks/new?sourceAssessmentSessionId=${assessment.id}`);
    ...
    const evidencePage = await evidenceContext.newPage(); observeRequests(evidencePage);
    ...
    for (const item of await rows('soa_items', { soa_register_id: soa.id })) {
      ...
      if (!matches) {
        ...
        await navigate(page, `/app/soa/${soa.id}`); await page.getByRole('button', { name: `Review ${item.control_code} ${item.control_title}`, exact: true }).click();
    ...
    if (diagnosticPage && !diagnosticPage.isClosed()) {
      await writeFile(path.join(dir, 'failure-diagnostic.json'), JSON.stringify({ url: diagnosticPage.url(), body: await diagnosticPage.locator('body').innerText().catch(() => ''), headings: await diagnosticPage.getByRole('heading').allTextContents().catch(() => []), browserErrors: browserErrors.map((message) => message.replace(/(?:https?|wss?):\/\/[^\s'"]+/g, (value) => { try { const url = new URL(value); return url.origin + url.pathname; } catch { return '[URL]'; } })), browserRequests: browserRequests.slice(-80) }, null, 2), { mode: 0o600 });
      await diagnosticPage.screenshot({ path: path.join(dir, 'failure-diagnostic.png'), fullPage: true }).catch(() => undefined);
    }
    throw error;
  } finally { for (const session of sessions) await writeFile(session.file, JSON.stringify(await session.context.storageState()), { mode: 0o600 }).catch(() => undefined); await browser?.close(); await release(); }
}
